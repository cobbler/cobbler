...
class AutoInstallationGen:
    ...
    def __generator_factory(
        ...
        if autoinstaller_type == "legacy":
            return LegacyGenerator(api=self.api, templar=self.templar)
        raise ValueError("Unknown template type selected!")

    def generate_autoinstall(
        self,
        obj: "BootableItem",
        autoinstaller_type: str = "",
    ...
    def generate_autoinstall_for_profile(
        ...
        """
        profile = self.api.find_profile(return_list=False, name=profile_name)
        if profile is None or isinstance(profile, list):
            return "# profile not found or ambigous match"
    ...
    def generate_autoinstall_for_system(
        ...
        if system_obj is None or isinstance(system_obj, list):
            return "# system not found or ambigous match"
