             ...
             if my_int is None:
                ...
                if len(data["interfaces"].keys()) >= 1:
                   for (iname, idata) in data["interfaces"].iteritems():
                      if idata["management"] == True and idata["interface_type"] in ["bond","bridge"]:
                         # bonded/bridged management interface
                         mgmt_ints_multi.append(iname)
                      if idata["management"] == True and idata["interface_type"] not in ["bond","bridge","bond_slave","bridge_slave"]:
                ...
                if len(mgmt_ints_multi) == 1 and len(mgmt_ints) == 0:
                   ...
                   for (iname, idata) in data["interfaces"].iteritems():
                      if idata["interface_type"] in ["bond_slave","bridge_slave"] and idata["interface_master"] == mgmt_ints_multi[0]:
