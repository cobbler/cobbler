...
def run(api, args, logger):
    ...
    if not str(settings.salt_remove_old_certs_automatically).lower() in [ "1", "yes", "y", "true"]:
        return 0
    ...
    if rc != 0:
        if logger is not None:
            logger.warning("salt-key removal for %s failed", name)
