...
def run(api,args,logger):
   ...
   for file in target["boot_files"].keys():
     ...
     try:
       ...
       api.log("copied file %s to %s for %s" % (target["boot_files"][file],file_dst,name))
     except:
       logger.error("failed to copy file %s to %s for %s" % (target["boot_files"][file],file_dst,name))
       return 1
