...
class ImportUbuntuManager:
    ...
    def check_for_signature(self,path,cli_breed):
       ...
       for signature in signatures:
           ...
           if os.path.exists(d):
               self.logger.info("Found an Ubuntu compatible signature: %s" % signature)
    ...
    def get_valid_os_versions(self):
        return ["dapper", "hardy", "lucid", "karmic", "maverick", "natty",] 
...
def get_import_manager(config,logger):
    return ImportUbuntuManager(config,logger)
