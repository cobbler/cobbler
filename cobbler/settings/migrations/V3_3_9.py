"""
Migration from V3.3.8 to V3.3.9
"""
...
from cobbler.settings.migrations import V3_3_8

# schema identical to V3_3_8
schema = V3_3_8.schema


def validate(settings: dict) -> bool:
    """
    Checks that a given settings dict is valid according to the reference V3.3.8 schema ``schema``.
...
def migrate(settings: dict) -> dict:
    """
    Migration of the settings ``settings`` to version V3.3.9 settings

    :param settings: The settings dict to migrate
    :return: The migrated dict
    """

    if not V3_3_8.validate(settings):
        raise SchemaError("V3.3.8: Schema error while validating")
