...
schema = Schema(
    {
        ...
        Optional("nsupdate_mgm_txt", True): bool,
        Optional(
            "nsupdate_tsig",
            default={
                "localhost.": {
                    "algorithm": "hmac-sha512",
                    "key": ["cobbler_update_key.", "hvnK54HFJXFasHjzjEn09ASIkCOGYSnofRq4ejsiBHz3udVyGiuebFGAswSjKUxNuhmllPrkI0HRSSmM2qvZug=="]
                },
            },
        ): dict,
...
def validate(settings: dict) -> bool:
    """
    Checks that a given settings dict is valid according to the reference V3.3.7 schema ``schema``.
...
def migrate(settings: dict) -> dict:
    ...
    if not V3_3_6.validate(settings):
        raise SchemaError("V3.3.6: Schema error while validating")
