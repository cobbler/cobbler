...
def glob(*args: str, **kwargs: Any) -> List[str]:
    """
    Find all files recursive to the given regex. Same arguments as "glob.glob" in the standard library.

    :returns: A list of paths that match the given regex.
    """
    ...
    if strip_prefix:
        ...
        for result in results:
            new_results.append(result.lstrip(strip_prefix))
        results = new_results
...
class CobblerBuildPy(build_py):
    ...
    def copy_manpages(self) -> None:
        ...
        man1_source = glob("docs/_build/man/*.1")
        man5_source = glob("docs/_build/man/*.5")
        man8_source = glob("docs/_build/man/*.8")
