...
When deploying a profile to multiple machines, certain settings may need to be different between machines.  Changing kernel options is fairly straightforward, although sometimes kickstarts must also change.  A good example of this is that the network settings (such as a static IP) need to be different between machines.  One traditional way to do this is with kickstart templating.
...
What this does is allow per-profile and per-system variables to be interpolated into the kickstart file.  For instance, with the "wordoftheday" example, anywhere "TEMPLATE::wordoftheday" occurs in the kickstart file will be replaced with the string "elevator".  

This only works for kickstart files with filesystem locations.  URL based kickstarts (NFS, HTTP) are
not templated.  Template variables are substituted each time "cobbler sync" is run.
