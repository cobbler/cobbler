...
koan --replace-self --with-extreme-prejudice --server=host --profile=profile_name
...
For re-kickstarting ('--replace-self'), this means reprovisioning the system, blowing away any current data and replacing it with the results of a network install.  The --withextremeprejudice argument is mandatory, as this command is *highly* destructive.  Do not issue it lightly.
