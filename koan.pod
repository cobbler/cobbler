...
koan --server=<host> [--virt|--replace-self|--display] [--profile=<name>|--system=<name>] [--add-reinstall-entry] [--autonet] [--virt-name=<name>] [--virt-path=<path>] [--virt-type=<type>] [--virt-graphics]
...
Tells koan to configure the installer's networking on the kernel command line using ip=/netmask=/gateway=, etc..   First, koan will download the kickstart configuration and check for a network --bootproto=static method.  If that fails and --replace-self has been specified, koan will use the current system's network configuration, as found using iproute2.  This option therefore makes koan installations more usable in environments that do not offer DHCP.  
...
If you want to just add an entry to grub (for example, to allow reinstalling
the system later), add the additional argument "--add-reinstall-entry" to the command line and it will not become the default for the next boot.  Only one reinstall entry can be added to grub; re-running this command will replace the previous entry.
...
To build a CD using Fedora as a base image, see instructions at:
