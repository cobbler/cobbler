...
def test_copy(
    ...
    for key, value in profile_collection.indexes.items():
        ...
        attr_val1 = profile_collection._get_index_property(profile1, key)
        attr_val2 = profile_collection._get_index_property(profile2, key)
...
def test_rename(
    ...
    for key, value in profile_collection.indexes.items():
        indx_prop = cobbler_api.settings().memory_indexes["profile"][key]
        attr_val = profile_collection._get_index_property(profile1, key)
...
def test_update_profile_indexes(
    ...
    distro1 = create_distro()
    distro2 = create_distro("test_update_profile_indexes_new")
    profile1 = create_profile(name)
    profile2 = profile.Profile(cobbler_api)
    ...
    assert profile_collection.indexes["distro"][profile1.distro.name] == {
        profile1.name,
        profile2.name,
    }
...
def test_remove_from_indexes(
    ...
    assert profile_collection.indexes["distro"][profile1.distro.name] == {profile2.name}
    assert profile_collection.indexes["arch"][profile1.arch.value] == {profile2.name}
...
def test_find_by_indexes(
    ...
    kargs12 = {"arch": profile1.arch.value}
...
def test_to_string(cobbler_api: CobblerAPI, profile_collection: profiles.Profiles):
    """
    TODO
    """
    # Arrange
    name = "to_string"
    item1 = profile.Profile(cobbler_api)
    item1.name = name
    profile_collection.add(item1)

    # Act
    result = profile_collection.to_string()
